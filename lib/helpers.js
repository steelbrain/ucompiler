...
function scanFiles(path, _ref) {
  ...
  if (path.indexOf('*') === -1) {
    // Non-Glob
    const absPath = _path2.default.isAbsolute(path) ? path : _path2.default.join(path, root);
    ...
    // Glob
    return _glob2.default.sync(path, { cwd: root, ignore: ignored });
  }
}
