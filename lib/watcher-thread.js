...
const Process = new (require('childprocess-promise'))()
...
Process.on('SCAN', function(Job){
  ...
  Job.Response = WorkerThread.scanDir(Info.Path, Info.Ignore)
})
Process.on('COMPILE', function(Job){
  const Info = Job.Message
  Job.Response = UCompiler.compileFile(Info.Path, Info.Options)
})
