...
function findFiles(pathGiven, ignoredGiven, options) {
  ...
  if (pathGiven === null) {
    ...
    const path = (0, _common.normalizePath)(_path2.default.isAbsolute(pathGiven) ? pathGiven : _path2.default.join(options.cwd, pathGiven));
    const baseName = _path2.default.basename(path);
    if ((0, _common.isIgnored)(baseName, path, ignored)) {
      return [];
    }
    ...
    if (stat.isFile()) {
      return [{
        relativePath: (0, _common.normalizePath)(_path2.default.relative(options.cwd, path)),
        absolutePath: path,
        fileName: baseName
      }];
...
function findFilesBase(path, ignored, _ref, validateCallback) {
  ...
  _fs2.default.readdirSync(path).forEach(function (entryName) {
    const absolutePath = (0, _common.normalizePath)(_path2.default.join(path, entryName));
    const relativePath = (0, _common.normalizePath)(_path2.default.relative(root, absolutePath));
...
function findFilesGlob(path, ignored, _ref3) {
  ...
  return findFilesBase(root, ignored, { root: root, config: config }, function (relative, _, stat) {
    return stat.isDirectory() || (0, _micromatch.isMatch)(relative, path);
  });
}
