...
class UCompiler {
  ...
  static defaultError(error) {
    let col = 0, line = 0, trace = ''
    if (error.loc) {
      line = error.loc.line || error.loc.row || 0
      col = error.loc.column || error.loc.col || 0
    } else if (error.location) {
      line = error.location.line || error.location.row || 0
      col = error.location.column || error.location.col || 0
    ...
    col = col || error.col || error.row || error.column || 0
    trace = trace || error.codeFrame || error.trace || ''

    console.error('Error caught on line', line, 'col', col, ':', error.message, "\n" + trace)
  }
}
